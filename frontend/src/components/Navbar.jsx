...
export default function DriverView() {
  ...
  // Auto-filled from localStorage — no manual input needed
  ...
  const fetchActive = async () => {
    try {
      ...
      if (active) {
        const checkin = new Date(active.checkin_time)
        const now = new Date()
        const elapsed = Math.floor((now - checkin) / 60000)
        setActiveEvent({ ...active, elapsed_minutes: elapsed, free_time_remaining: Math.max(0, active.free_time_minutes - elapsed) })
        setElapsed(elapsed)
      } else {
        setActiveEvent(null)
      }
